...
class CatatanObservasiPostpartumController extends BaseController
{
    public function dataCatatanObservasi()
    {
        ...
        $url = $this->api_url . '/catatan-observasi-ranap-postpartum';
    ...
    {
        if (session()->has('jwt_token')) {
            $title = 'Tambah Catatan Observasi Post Partum';
    ...
    public function submitTambahCatatanObservasi()
    {
        if (session()->has('jwt_token')) {
            ...
            $postData = [
                ...
                'perdarahan' => $this->request->getPost('perdarahan'),
    ...
    public function editCatatanObservasiPostpartum($noRawat)
    {
        ...
        $token = session()->get('jwt_token');
        $title = 'Edit Observasi Ranap Post Partum';

        $url = $this->api_url . '/catatan-observasi-ranap-postpartum/' . $noRawat;
   ...
   public function hapusCatatanObservasiPostpartum($noRawat)
    {
        if (session()->has('jwt_token')) {
            $token = session()->get('jwt_token');
            $delete_url = $this->api_url . '/catatan-observasi-ranap-postpartum/' . $noRawat;
    ...
    public function submitFromRawatinapToCatatanObservasi($nomor_rawat)
    {
        ...
        // Step 4: Siapkan data untuk prefill
        $prefill = [
            'no_rawat'     => $rawatinap['nomor_rawat'] ?? $nomor_rawat,
            'nip'          => '', // Petugas pilih sendiri

...
foreach (['is_identitas_sesuai', 'is_resiko_aspirasi', 'is_resiko_hilang_darah'] as $boolField) {
...
<div class="max-w-[85rem] py-6 lg:py-3 px-8 mx-auto">
    <div class="bg-white rounded-xl shadow-sm p-4 sm:p-7 dark:bg-slate-900 border border-gray-100 dark:border-gray-800">
        ...
        <form action="<?= $modul_path . $form_action ?>" id="myForm" onsubmit="return validateForm()" method="post">
            ...
            <div class="mb-5 sm:block md:flex items-center">
                <label class="<?= $labelLeft ?>">Waktu Sign In <span class="text-red-500">*</span></label>
                <input type="datetime-local" name="waktu_signin"
                       value="<?= esc(substr(str_replace(' ', 'T', $baris['waktu_signin'] ?? ''), 0, 16)) ?>"
            ...
            <div class="mb-5 sm:block md:flex items-center">
                <label class="<?= $labelLeft ?>">Alergi <span class="text-red-500">*</span></label>
                <input type="text" name="alergi"
                       value="<?= esc($baris['alergi'] ?? '') ?>"
                       required class="<?= $inputClass ?> lg:w-1/4">
            </div>

            <div class="mb-5 sm:block md:flex items-center">
                <label class="<?= $labelLeft ?>">Risiko Aspirasi <span class="text-red-500">*</span></label>
                <select name="is_resiko_aspirasi" required class="<?= $stdClass ?> lg:w-1/4">
                    <option value="">— Pilih —</option>
                    <option value="1" <?= ($baris['is_resiko_aspirasi'] ?? '') == '1' ? 'selected' : '' ?>>Ya</option>
                    <option value="0" <?= isset($baris['is_resiko_aspirasi']) && $baris['is_resiko_aspirasi'] == '0' ? 'selected' : '' ?>>Tidak</option>
                </select>

                <label class="<?= $labelRight ?>">Rencana Antisipasi Aspirasi <span class="text-red-500">*</span></label>
                <input type="text" name="rencana_aspirasi"
                       value="<?= esc($baris['rencana_aspirasi'] ?? '') ?>"
                       required class="<?= $inputClass ?> lg:w-1/4">
            </div>

            <div class="mb-5 sm:block md:flex items-center">
                <label class="<?= $labelLeft ?>">Risiko Hilang Darah <span class="text-red-500">*</span></label>
                <select name="is_resiko_hilang_darah" required class="<?= $stdClass ?> lg:w-1/4">
                    <option value="">— Pilih —</option>
                    <option value="1" <?= ($baris['is_resiko_hilang_darah'] ?? '') == '1' ? 'selected' : '' ?>>Ya</option>
                    <option value="0" <?= isset($baris['is_resiko_hilang_darah']) && $baris['is_resiko_hilang_darah'] == '0' ? 'selected' : '' ?>>Tidak</option>
                </select>

                <label class="<?= $labelRight ?>">Jalur IV Line <span class="text-red-500">*</span></label>
                <input type="text" name="jalur_iv_line"
                       value="<?= esc($baris['jalur_iv_line'] ?? '') ?>"
                       required class="<?= $inputClass ?> lg:w-1/4">
            </div>

            <div class="mb-5 sm:block md:flex items-center">
                <label class="<?= $labelLeft ?>">Rencana Antisipasi Hilang Darah <span class="text-red-500">*</span></label>
                <input type="text" name="rencana_hilang_darah"
                       value="<?= esc($baris['rencana_hilang_darah'] ?? '') ?>"
                       required class="<?= $inputClass ?> lg:w-1/4">
            </div>

            <div class="mb-5 sm:block md:flex items-center">
                <label class="<?= $labelLeft ?>">Kesiapan Anestesi <span class="text-red-500">*</span></label>
                <select name="id_kesiapan_anestesi" required class="<?= $stdClass ?> lg:w-1/4">
                    <option value="">— Pilih —</option>
                    <?php foreach ($options['kesiapan_anestesi'] as $o): ?>
                        <option value="<?= esc($o['id_kesiapan']) ?>"
                            <?= ($baris['id_kesiapan_anestesi'] ?? '') == $o['id_kesiapan'] ? 'selected' : '' ?>>
                            <?= esc($o['nama_kesiapan']) ?>
                        </option>
                    <?php endforeach; ?>
                </select>

                <label class="<?= $labelRight ?>">Rencana Antisipasi Kesiapan <span class="text-red-500">*</span></label>
                <input type="text" name="rencana_kesiapan_anestesi"
                       value="<?= esc($baris['rencana_kesiapan_anestesi'] ?? '') ?>"
                       required class="<?= $inputClass ?> lg:w-1/4">
            </div>
            ...
            <div class="mb-5 sm:block md:flex items-center">
                <label class="<?= $labelLeft ?>">Perawat OK <span class="text-red-500">*</span></label>
                <div class="flex gap-x-2 lg:w-1/4">

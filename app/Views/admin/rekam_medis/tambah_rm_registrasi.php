...
<?= view('components/modal/modalunit', ['excludeIds' => [22]]) ?>
...
<div class="max-w-[85rem] py-6 lg:py-3 px-8 mx-auto">
    <div class="bg-white rounded-xl shadow p-4 sm:p-7 dark:bg-slate-900">
        ...
        <form action="<?= "$modul_path$form_action" ?>" id="myForm" onsubmit="return validateForm()" method="post">
            ...
            <input type="hidden" name="unit"         id="unit"         value="<?= esc($baris['unit'] ?? '') ?>">
            ...
            <div class="mb-5 sm:block md:flex items-center">
                ...
                <div class="flex gap-x-2 lg:w-1/4">
                    <input type="text" id="no_rm_display" readonly required placeholder="Klik cari pasien..."
                ...
                <input type="text" id="nm_pasien" readonly placeholder="Terisi otomatis..."
                       value="<?= esc($baris['id_pasien_nama'] ?? '') ?>"
                       class="<?= $readonlyClass ?>">
            </div>

            <!-- Dokter -->
            <div class="mb-5 sm:block md:flex items-center">
                <label class="<?= $labelLeft ?>">Dokter <span class="text-red-600">*</span></label>
                <div class="flex gap-x-2 lg:w-1/4">
            ...
            <!-- Unit | Biaya Registrasi (autofill dari unit, editable) -->
            <div class="mb-5 sm:block md:flex items-center">
                <label class="<?= $labelLeft ?>">Unit <span class="text-red-600">*</span></label>
                <div class="flex gap-x-2 lg:w-1/4">
                    <input type="text" id="nama_unit_display" readonly required placeholder="Klik cari unit..."
                           value="<?= esc($baris['nama_unit'] ?? '') ?>"
                           onclick="open_modalUnit()" class="<?= $inputClass ?>">
                ...
                <input type="number" name="biaya_registrasi" id="biaya_registrasi"
                       value="<?= (int) ($baris['biaya_registrasi'] ?? 0) ?>" min="0"
                       class="<?= $readonlyClass ?> bg-white cursor-text lg:w-1/4">
            </div>
            ...
            <!-- Sisa field dari konfig (tanggal_reg, no_telepon, jenis_bayar, status_*) -->
...
function autofillUnitFields(item) {
    document.getElementById('unit').value               = item.id_unit             ?? '';
    document.getElementById('nama_unit_display').value  = item.nama_unit           ?? '';
    document.getElementById('biaya_registrasi').value   = parseInt(item.biaya_registrasi_baru ?? 0) || 0;
}
...
function validateForm() {
    var checks = [
        ...
        ['unit',        'Pilih unit terlebih dahulu.'],

...
<div class="max-w-[85rem] py-6 lg:py-3 px-8 mx-auto">
    <div class="bg-white rounded-xl shadow-sm p-4 sm:p-7 dark:bg-slate-900 border border-gray-100 dark:border-gray-800">
        ...
        <form action="<?= $modul_path . $form_action ?>" id="myForm" onsubmit="return validateForm()" method="post">
            ...
            <input type="hidden" name="nomor_reg"           id="nomor_reg"           value="<?= $val('nomor_reg') ?>">
            <input type="hidden" name="kode_dokter_perujuk" id="kode_dokter_perujuk" value="<?= $val('kode_dokter_perujuk') ?>">

            <!-- No. Permintaan + No. Registrasi -->
            ...
            <div class="mb-5 mt-4">
                <div class="flex justify-between items-center mb-3">
                    ...
                    <button type="button" onclick="open_modalItemRad()" class="<?= $addBtnClass ?>">
                        <?= $plusIcon ?> Pilih Item
                    </button>
                </div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (itemTerpilih.length > 0) {
            itemTerpilih.forEach(item => { _itemRadSelected[item.id_item] = item; });

...
final class SkorStewardController extends ControllerTemplate
{
    public function __construct()
    {
        parent::__construct(
            ...
            [
                ...
                [SHOW, REQUIRED, I::INDEX,  'skor_kesadaran',     'Skor Kesadaran'],
                [SHOW, REQUIRED, I::INDEX,  'skor_respirasi',     'Skor Respirasi'],
                [SHOW, REQUIRED, I::INDEX,  'skor_motorik',       'Skor Motorik'],
    ...
    private function fetchOptions(): array
    {
        $db = $this->model->db;
        return [
            'kesadaran' => $db->table('operasi.ref_steward_kesadaran')
                ->select('id_kesadaran, nama_skala, nilai')->orderBy('nilai')->get()->getResultArray(),
            'respirasi' => $db->table('operasi.ref_steward_respirasi')
                ->select('id_respirasi, nama_skala, nilai')->orderBy('nilai')->get()->getResultArray(),
            'motorik'   => $db->table('operasi.ref_steward_motorik')
                ->select('id_motorik, nama_skala, nilai')->orderBy('nilai')->get()->getResultArray(),
        ];
    }
    ...
    public function create_page(): string
    {
        ...
        return view('admin/operasi/tambah_skor_steward',

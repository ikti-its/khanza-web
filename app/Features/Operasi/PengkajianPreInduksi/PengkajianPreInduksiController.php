...
final class PengkajianPreInduksiController extends ControllerTemplate
{
    ...
    private function fetchOptions(): array
    {
        $db = $this->model->db;
        return [
            'jenis_airway' => $db->table('operasi.ref_jenis_airway')
                ->select('id_jenis, nama_jenis')->get()->getResultArray(),
            'posisi'       => $db->table('operasi.ref_posisi_pasien')
                ->select('id_posisi, nama_posisi')->get()->getResultArray(),
            'premedikasi'  => $db->table('operasi.ref_premedikasi')
                ->select('id_premedikasi, nama_premedikasi')->get()->getResultArray(),
            'induksi'      => $db->table('operasi.ref_induksi')
                ->select('id_induksi, nama_induksi')->get()->getResultArray(),
        ];
    }
    ...
    private function insertAirwayList(int $idPengkajian, array $airwayList): void
    {
        ...
        if (!empty($batchAirway)) {
            (new \App\Features\Operasi\PengkajianPreInduksiAirway\PengkajianPreInduksiAirwayModel())->insertBatch($batchAirway);
        }
    }
    ...
    public function create_page(): string
    {
        ...
        return view('admin/operasi/tambah_pengkajian_pre_induksi',
    ...
    public function update_page(int|string $id): string
    {
        ...
        return view('admin/operasi/tambah_pengkajian_pre_induksi',
            $this->buildViewData($jadwal, $record, '/submitedit/' . $id, $this->fetchAirway((int) $id)));
    }
    ...
    public function update(int|string $id): string|RedirectResponse
    {
        ...
        try {
            ...
            (new \App\Features\Operasi\PengkajianPreInduksiAirway\PengkajianPreInduksiAirwayModel())->where('id_pengkajian', $id)->delete();

...
final class CatatanAnestesiSedasiController extends ControllerTemplate
{
    public function __construct()
    {
        parent::__construct(
            ...
            [
                ...
                [SHOW, REQUIRED, I::DTIME,  'waktu_catatan',        'Waktu Catatan'],
    ...
    private function fetchOptions(): array
    {
        $db = $this->model->db;
        return [
            'kesadaran'      => $db->table('operasi.ref_kesadaran')
                ->select('id_kesadaran, nama_kesadaran')->get()->getResultArray(),
            'golongan_darah' => $db->table('darah.golongan_darah')
                ->select('id_golongan_darah, nama_golongan_darah')->get()->getResultArray(),
            'rhesus'         => $db->table('darah.rhesus')
                ->select('id_rhesus, kode_rhesus')->get()->getResultArray(),
            'asa'            => $db->table('operasi.ref_angka_asa')
                ->select('id_asa, nama_asa')->get()->getResultArray(),
            'jenis_sedasi'   => $db->table('operasi.ref_jenis_sedasi')
                ->select('id_jenis_sedasi, nama_sedasi')->get()->getResultArray(),
            'alat'           => $db->table('operasi.ref_alat_anestesi')
                ->select('id_alat, nama_alat')->get()->getResultArray(),
            'monitoring'     => $db->table('operasi.ref_monitoring_anestesi')
                ->select('id_monitoring, nama_monitoring')->get()->getResultArray(),
        ];
    }
    ...
    private function insertAlatAndMonitoring(int $idCatatan, array $alatList, array $monitoringList): void
    {
        ...
        foreach ($alatList as $row) {
            ...
            if ($idAlat === 0 || ($isDigunakan === null && $keterangan === null)) continue;
        ...
        if (!empty($batchAlat)) {
            (new \App\Features\Operasi\CatatanAnestesiSedasiAlat\CatatanAnestesiSedasiAlatModel())->insertBatch($batchAlat);
        }
        ...
        foreach ($monitoringList as $row) {
            ...
            if ($idMonitoring === 0 || ($isDigunakan === null && $keterangan === null)) continue;
        ...
        if (!empty($batchMonitoring)) {
            (new \App\Features\Operasi\CatatanAnestesiSedasiMonitoring\CatatanAnestesiSedasiMonitoringModel())->insertBatch($batchMonitoring);
        }
    }
    
    private function buildViewData(array $jadwal, array $record, string $formAction, ?int $idCatatan = null): array
    {
        return [
    ...
    public function update(int|string $id): string|RedirectResponse
    {
        ...
        try {
            ...
            (new \App\Features\Operasi\CatatanAnestesiSedasiAlat\CatatanAnestesiSedasiAlatModel())->where('id_catatan_anestesi', $id)->delete();
            (new \App\Features\Operasi\CatatanAnestesiSedasiMonitoring\CatatanAnestesiSedasiMonitoringModel())->where('id_catatan_anestesi', $id)->delete();

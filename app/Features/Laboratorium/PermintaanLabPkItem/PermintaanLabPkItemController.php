...
final class PermintaanLabPkItemController extends ControllerTemplate
{
    ...
    private function generateNomorPermintaan(): string
    {
        ...
        return generateNextNoPermintaanPk(is_string($lastNo['no_permintaan'] ?? null) ? $lastNo['no_permintaan'] : null);
    }
    ...
    private function fetchItemTerpilih(int $idPermintaanLab): array
    {
        ...
        $builderParams = $this->model
            ...
            ->whereIn('pkp.id_permintaan_pk_item', $itemIds)
        ...
        foreach ($parameters as $param) {
            $idItem                   = (string) ($param['id_permintaan_pk_item'] ?? '');
            $groupedParams[$idItem][] = $param;
        }

        foreach ($items as &$item) {
            $item['parameter'] = $groupedParams[(string) ($item['id_permintaan_pk_item'] ?? '')] ?? [];
        }
        unset($item);
    ...
    private function deleteItemsAndParameters(
        ...
        $oldItemIds = array_column($oldItems, 'id_permintaan_pk_item');

        if ($oldItemIds !== []) {
            $modelParameter->whereIn('id_permintaan_pk_item', $oldItemIds)->delete();
        }
    ...
    private function fetchStatusFilters(): array
    {
        $builderCount = $this->model
            ...
            ->where('id_kategori_lab', ID_KATEGORI_PK)
    ...
    public function print(int|string $id): string
    {
        ...
        return view('Views/components/cetak/cetak_permintaan_lab_pk', [
            'header' => $header,

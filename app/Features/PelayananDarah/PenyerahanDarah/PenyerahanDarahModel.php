...
final class PenyerahanDarahModel extends ModelTemplate
{
     ...
     */
    public function validasiDanHitungKuota(int $idPermintaan, ?array $stokDarahTerpilih): int
    {
        $totalDiminta = (int)$this->db->table('pelayanan_darah.permintaan_darah_detail')
        ...
        if ($totalDiminta <= 0) {
            throw new \RuntimeException("Gagal menyimpan! Jumlah kantong pada detail dokumen permintaan darah bernilai 0.");
        }

        $listIdPenyerahanLama = $this->db->table('pelayanan_darah.penyerahan_darah')
            ->where('id_permintaan', $idPermintaan)
        ...
        if (!empty($listIdPenyerahanLama)) {
            ...
            $totalSudahDiserahkan = (int)$this->db->table('pelayanan_darah.penyerahan_darah_detail')
        ...
        if ($totalSudahDiserahkan >= $totalDiminta) {
            throw new \RuntimeException("Gagal menyimpan! Seluruh kantong darah untuk permintaan ini sudah diserahkan sebelumnya.");
        }

        if ($akumulasiMasaDepan > $totalDiminta) {
            $sisaKuota = $totalDiminta - $totalSudahDiserahkan;
            throw new \RuntimeException("Gagal menyimpan! Jumlah darah yang dimasukkan melebihi batas kuota dokter. Sisa darah yang boleh diserahkan hanya: {$sisaKuota} kantong.");
        }
    ...
    public function sinkronisasiStatusPermintaan(int $idPermintaan): void
    {
        ...
        $listIdPenyerahan = $this->db->table('pelayanan_darah.penyerahan_darah')
            ->where('id_permintaan', $idPermintaan)
            ->select('id_penyerahan')
            ->get()
            ->getResultArray();
        ...
        if (!empty($listIdPenyerahan)) {
            ...
            $totalDiserahkan = (int)$this->db->table('pelayanan_darah.penyerahan_darah_detail')
        ...
        $this->db->table('pelayanan_darah.permintaan_darah')
            ->where('id_permintaan', $idPermintaan)

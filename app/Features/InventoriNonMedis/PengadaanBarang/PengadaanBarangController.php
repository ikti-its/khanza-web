...
final class PengadaanBarangController extends ControllerTemplate
{
    ...
    /** @throws \CodeIgniter\Database\Exceptions\DatabaseException */
    private function validate_qty_terhadap_pengajuan(
        int $id_pengajuan,
        array $detail_ids,
        array $detail_qty,
        int $exclude_id_pengadaan = 0,
    ): null|string {
        if ($id_pengajuan <= 0)
            return null;
        ...
        for ($i = 0; $i < count($detail_ids); $i++) {
            ...
            $pjd = $this->guarded(
                $db
                    ->table('inventori_non_medis.pengajuan_barang_detail pjd')
    ...
    public function create(): string|RedirectResponse
    {
        ...
        $postData = [
            ...
            'id_suplier'   => (int) ($this->request->getPost('id_suplier') ?? 0) > 0
                ? (int) $this->request->getPost('id_suplier')
                : 0,
        ...
        $tanggalPengadaan = $postData['tanggal'] ?? null;
        /** @var string|null $tanggalPengadaan */
        $postData['no_pengadaan']               = generateNextNoPengadaanBarang($lastNo, $tanggalPengadaan);

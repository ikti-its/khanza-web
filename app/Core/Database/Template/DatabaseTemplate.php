...
class DatabaseTemplate extends Migration
{
    ...
    private function add_primary_key(): void
    {
        ...
        assert(array_key_exists($pk, $this->fields), "Primary key '{$pk}' is not defined in fields: {$field_names}");

        assert(
            !$this->fields[$pk]['null'],
            "Primary key field '{$pk}' cannot be nullable. " . "Schema : {$this->schema}, Table : {$this->table}",
        );
        ...
        foreach ($this->foreign_keys as $fk) {
            ...
            foreach ($fields as $field) {
                assert(
                    ...
                    "Foreign key field {$field} not found in fields:" . implode(', ', array_keys($this->fields)),
                );
            }
    ...
    private function set_fk_auto(): void
    {
        ...
        foreach ($fks as $fk) {
            ...
            for ($i = 0; $i < count($fields); $i++) {
                ...
                $ref_def['type'] = (string) preg_replace(
    ...
    private function adjust_db_key_sequence(): void
    {
        ...
        ");
        assert($seq_result instanceof BaseResult, 'Failed to query pg_get_serial_sequence');
        $seq_row = $seq_result->getRowArray();

        if (!isset($seq_row['seq_name']))
            return;
        $this->db->query("
    ...
    public function dependencies(): array
    {
        ...
        foreach ($this->foreign_keys as $fk) {
            ...
            assert($ref_table_name !== static::class, "Foreign key refer to itself : {$ref_table_name}");
